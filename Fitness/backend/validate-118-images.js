...
// Free-exercise-db base URL
const FDB = 'https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/';

// Map every single one of the 118 exercises to a verified, unique image URL
...
async function testAll() {
  console.log('Checking all 118 exercises are present in mapping...');
  const missing = [];
  ALL_118_EXERCISES.forEach(ex => {
    if (!EXERCISE_IMAGES[ex.name]) missing.push(ex.name);
  });
  if (missing.length > 0) {
    console.error('Missing exercises:', missing);
    process.exit(1);
  }
  console.log('All 118 exercises present!');

  const urls = Object.values(EXERCISE_IMAGES);
  const uniqueUrls = new Set(urls);
  console.log(`Total URLs: ${urls.length}, Unique URLs: ${uniqueUrls.size}`);
  if (urls.length !== uniqueUrls.size) {
    console.error('There are duplicates in the URLs!');
    const counts = {};
    urls.forEach(u => counts[u] = (counts[u] || 0) + 1);
    Object.entries(counts).filter(([_, c]) => c > 1).forEach(([u, c]) => console.log('Duplicate:', c, u));
    process.exit(1);
  }
  console.log('100% of URLs are unique! Now testing HTTP status for each URL in parallel...');

  const checks = Object.entries(EXERCISE_IMAGES).map(([name, url]) => {
    return new Promise((resolve) => {
      ...
      client.get(url, { method: 'HEAD' }, res => {
        if (res.statusCode === 200 || res.statusCode === 302) {
          ...
          console.error(`FAIL: ${name} -> ${url} (HTTP ${res.statusCode})`);
          resolve({ name, url, ok: false, status: res.statusCode });
        }
      }).on('error', err => {
        console.error(`ERROR: ${name} -> ${url} (${err.message})`);
        resolve({ name, url, ok: false, error: err.message });
      });
    });
  });
  ...
  console.log(`\nFinal Validation Results: ${passed} PASSED, ${failed} FAILED out of ${urls.length} URLs.`);
}
