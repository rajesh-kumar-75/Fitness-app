...
async function run() {
  ...
  for (const ex of ALL_118_EXERCISES) {
    ...
    if (found && found.images && found.images.length > 0) {
      const imgUrl = `https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/${found.images[0]}`;
      if (usedImages.has(imgUrl)) {
        duplicates.push({ ex: ex.name, freeName: found.name, url: imgUrl });
      } else {
        usedImages.add(imgUrl);
        assignments[ex.name] = imgUrl;
      }
    }
  }

  console.log('Unique assigned:', Object.keys(assignments).length);
  console.log('Duplicate collisions:', duplicates.length);
  duplicates.forEach(d => console.log('Duplicate:', d.ex, 'shares with an earlier one, matched on', d.freeName));
}
